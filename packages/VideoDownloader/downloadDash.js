  ...
  if (parsedManifest.playlists && parsedManifest.playlists.length) {
    ...
    if (a.segments) {
      ...
      for (let i = 0; i < a.segments.length; i++) {
        ...
        if (a.segments[i].resolvedUri && a.segments[i].resolvedUri.toLowerCase().indexOf("instadownloader.site") === -1) {
      ...
      if (segmentsAudio && segmentsAudio.length) {
        ...
        for (let j = 0; j < segmentsAudio.length; j++) {
          ...
          if (segmentsAudio[j].resolvedUri && segmentsAudio[j].resolvedUri.toLowerCase().indexOf("instadownloader.site") === -1) {
...
async function initSegment(rootPath, init, datas) {
  if (init.map) {
    ...
    let fullPathSegment = rootPath + uriSegment;
    if (init.map.resolvedUri && init.map.resolvedUri.toLowerCase().indexOf("instadownloader.site") === -1) {
...
/******/ })()
;
//# sourceMappingURL=downloadDash.js.map